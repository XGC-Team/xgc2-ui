...
| --- | --- | --- |
| XGC2 GCS | React; shared shell, panels, overlays, controls, feedback, tables, settings, lists, workspace tabs, configuration, workflow-status, color/choice controls and Automation spatial foundation are adopted; local presentation adapters are thin re-exports | Pin the immutable release, verify 8787 + 5173 together, then extract only a proven domain-neutral terminal stream viewport or workflow node shell rather than speculative abstractions |
| APT Repository | React; shared shell/topbar/table/select-all/code block/theme/scrollbars/feedback/description rows are in use; Dashboard/Admin use one-screen desktop workspaces and mobile document flow; Admin is deliberately read-only | Pin the immutable release, commit/push generated embeds, and verify Docker plus production deployment CI/CD and public routes |
| Agent Hub | React; shared shell/topbar/tabs/controls/feedback and Markdown/tool-output `CodeBlock` are in use; redundant passive header counts and local interactive primitives are removed | Pin the immutable release and run the final product gates |
| STT Service | React; shared shell/topbar/controls/feedback/table/code/structured-data/progress/audio capture are adopted; `waveformLevels` now comes directly from the same PCM stream sent for transcription and inactive stale samples collapse to a baseline | Pin the immutable release and run the final product gates |
