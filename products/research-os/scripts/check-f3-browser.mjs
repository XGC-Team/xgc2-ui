...
await context.addInitScript(project=>{localStorage.setItem('research-ui-project',project);localStorage.setItem('research-ui-locale','en');localStorage.setItem('research-ui-bottom','collapsed');localStorage.setItem('research.markPrompt.dockVisible','false')},project)
...
try{
 ...
 await review.getByLabel('Select the exact range in raw text',{exact:true}).evaluate(el=>{el.focus();el.setSelectionRange(0,'A strong conclusion.'.length);el.dispatchEvent(new Event('select',{bubbles:true}));document.dispatchEvent(new Event('selectionchange'))})
