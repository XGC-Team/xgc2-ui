...
import {
  ...
  flatOutline, getArrangement, indentOutlineItem, migrateCanvasV1toV2, moveOutlineItem, newCanvasEvidence,
  ...
  type ThinkingCanvas, type ThinkingCanvasV2,
...
test('migration keeps IDs, refs, anchors, content and extension fields', () => {
  const extended = { ...v1, future: { keep: true } }
  const v2 = migrateCanvasV1toV2(extended as ThinkingCanvas)
  assert.equal(v2.version, 2)
  assert.deepEqual(v2.nodes.map(n => n.id), v1.nodes.map(n => n.id))
  assert.equal(v2.nodes.find(n => n.id === 'c1')?.anchor, 'main.tex')
  assert.deepEqual(v2.nodes.find(n => n.id === 'i1')?.ref, { path: 'papers/foo.md', title: 'Foo' })
  assert.deepEqual((v2 as typeof extended).future, { keep: true })
})
test('migration seeds the outline once from chapter edges and y/x; loose ideas stay unarranged', () => {
  const v2 = migrateCanvasV1toV2(v1)
  const flat = flatOutline(getArrangement(v2, PRIMARY_OUTLINE))
...
test('migration never promotes unlabeled v1 edges into semantic or hierarchical relations', () => {
  const v2 = migrateCanvasV1toV2(v1)
  assert.ok(v2.edges.every(edge => edge.relation === undefined))
  assert.equal(v2.edges.length, v1.edges.length)
})
...
for (const [name, mutate] of [
  ...
  test(`invalid v2 rejected: ${name}`, () => {
    const v2 = migrateCanvasV1toV2(v1) as unknown as Record<string, unknown>
    assert.throws(() => parseEditableCanvas(JSON.stringify(mutate(v2))))
  })
}
...
test('outline reorder, indent and outdent operate on siblings without touching layout', () => {
  ...
  assert.deepEqual(v2.nodes.find(n => n.id === 'i1'), { ...v1.nodes.find(n => n.id === 'i1')! })
})
...
test('evidence keeps excerpt, observed digest and source note; unverifiable stays explicit', () => {
  ...
  const reopened = parseEditableCanvas(serializeCanvas(v2)) as ThinkingCanvasV2
...
test('writing constraints round-trip and disappear when emptied', () => {
  ...
  v2 = setNodeWriting(v2, 'i1', 'template', '会议模板')
  const reopened = parseEditableCanvas(serializeCanvas(v2)) as ThinkingCanvasV2
  assert.deepEqual(reopened.nodes.find(n => n.id === 'i1')?.writing, { purpose: '引出缺口', template: '会议模板' })
  v2 = setNodeWriting(v2, 'i1', 'purpose', '')
  v2 = setNodeWriting(v2, 'i1', 'template', '')
  assert.equal(v2.nodes.find(n => n.id === 'i1')?.writing, undefined)
})
test('v2 prompt uses the explicit outline, not coordinates; only labeled edges appear as relations', () => {
  let v2 = migrateCanvasV1toV2(v1)
  v2 = setEdgeRelation(v2, 2, 'supports')
  v2 = addNodeEvidence(v2, 'i1', newCanvasEvidence({ path: 'papers/a.md', digest: 'rev-1', excerpt: 'quoted' }))
  v2 = setNodeWriting(v2, 'i1', 'purpose', '引出缺口')
  // Move cards around: prompt must remain identical.
...
test('canvas reference nodes keep the anchor contract on v2 and do not enter outlines', () => {
  const v2 = migrateCanvasV1toV2(v1)
