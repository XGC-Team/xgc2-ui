...
try{
 ...
 const note=p.locator('[data-xgc-role="pdf-annotation-mark"][title="请调整这个公式与前后正文的间距，并保持公式编号右对齐。"]');await note.waitFor();await note.click();await p.getByText('区域批注',{exact:true}).waitFor();await p.getByRole('button',{name:'加入这次讨论',exact:true}).click();const draft=await p.locator('[data-xgc-role="agent-composer-input-editor"]').innerText();assert.ok(draft.includes('定位：region'));assert.ok(draft.includes('在设计确认前不要改正文'))
 await p.screenshot({path:'artifacts/pdf-inline-20260908/inline-region.png'})
