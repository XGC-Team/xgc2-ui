...
try {
 ...
 await page.getByRole('button',{name:'全局搜索',exact:true}).click();await page.getByRole('dialog',{name:'导航与命令',exact:true}).getByRole('button',{name:'切换深浅主题',exact:true}).click();assert.ok(await page.locator('html').evaluate(el=>el.classList.contains('dark')));await page.setViewportSize({width:1000,height:760});await page.screenshot({path:join(data,'workflow-dark.png')})
