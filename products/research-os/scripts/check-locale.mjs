...
await page.getByRole('combobox',{name:'Interface language',exact:true}).selectOption('zh');await page.getByRole('navigation',{name:'顶层菜单'}).getByRole('button',{name:'聊天',exact:true}).click();
// The native-agent composer is a vendor component: its label is not localized; the shell around it is.
await page.getByRole('textbox',{name:'Message the agent',exact:true}).waitFor();await page.getByText('Ask anything...',{exact:true}).waitFor();
