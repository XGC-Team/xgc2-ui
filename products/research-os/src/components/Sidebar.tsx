...
export function Sidebar({projects,onRefresh,loading,error}:{projects:Project[];onRefresh:()=>void;loading:boolean;error:string}) {
 const {activeNav,setActiveNav,projectId,setProjectId,enterWritingProject,openRightTab,openCanvas,closeSourceView,locale}=useWorkbench();const native=useNativeAgentSession()
 ...
 const newThread=(project:string)=>{closeSourceView();if(project)enterWritingProject(project);else setProjectId(project);native.newThread();setActiveNav('chat');setExpanded(s=>({...s,[project]:true}))}
