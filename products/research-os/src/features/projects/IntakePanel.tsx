...
export function IntakePanel({ scope, compact = false }: { scope: DraftScope; compact?: boolean }) {
  ...
  return <section aria-label={zh ? '资料投入' : 'Material intake'} className="my-3 space-y-2">
    {!compact && <>
      ...
      <p className="text-caption text-ink-3">{zh ? '文本保存到项目工作区；PDF 沿用全局归档接口。链接仅记录引用，不声称已抓取。投入队列仅在本次页面会话中保留。' : 'Text is stored in the project workspace; PDF uses the existing global archive. URLs are references, not fetched content. Queue receipts last for this page session.'}</p>
    </>}
    {visible.map(record => <div key={record.id} className="rounded-md bg-elevated p-2 text-caption" data-intake-state={record.state}>
      ...
      {record.state === 'accepted' && record.kind === 'pdf' && <p>{zh ? '原有 PDF 归档接口已接收；此接口未提供本面板可用的阅读定位，不伪造文件或预览。' : 'Existing PDF intake accepted it. No reader target is exposed to this panel; no preview is fabricated.'}</p>}
    </div>)}
