...
export default function PDFReader({pdf,onPDF,onTitle,onDraftChange}:{pdf:ManuscriptPDF;onPDF:(pdf:ManuscriptPDF)=>void;onQuote:(text:string,targetProject?:string)=>void;onTitle?:(title:string)=>void;onDraftChange?:(dirty:boolean)=>void}){
 const {locale,openSourceView,pdfFlash,requestReviewFeedback}=useWorkbench();const [document,setDocument]=useState<PDFDocumentProxy|null>(null),[page,setPage]=useState(1),[scale,setScale]=useState(1),[error,setError]=useState(''),[quote,setQuote]=useState(''),[comment,setComment]=useState(''),[busy,setBusy]=useState(false),[reload,setReload]=useState(0),[notes,setNotes]=useState<{id:string;page:number;body:string}[]>([])
 ...
 useEffect(()=>{setDocument(null);setError('');setQuote('');setComment('');setAnchor(null);setSelectedNote(null);setDraftRect(null);attempt.current=null;setNotes([]);const place=readReadingPlace(pdf.workspace);setPage(place&&place.buildId===pdf.buildId&&place.digest===pdf.digest&&place.path===pdf.path?place.page:1);if(!pdf)return;const task=getDocument(pdf.url);let alive=true;task.promise.then(doc=>{if(alive)setDocument(doc)}).catch(e=>{if(alive)setError(e.message)});return()=>{alive=false;void task.destroy()}},[pdf])
 ...
 useEffect(()=>{if(!pdf?.workspace||!Number.isInteger(page)||page<1)return;writeReadingPlace(pdf.workspace,{path:pdf.path,buildId:pdf.buildId,digest:pdf.digest,page})},[pdf,page])
 async function annotate(){if(!pdf||!anchor||!comment.trim())return;setBusy(true);setError('');const submitted=comment.trim();const submittedAnchor=anchor;const body=encodeAnnotation({...submittedAnchor,pdf:{workspace:pdf.workspace,path:pdf.path,digest:pdf.digest}},submitted);if(attempt.current?.body!==body)attempt.current={body,key:crypto.randomUUID()};try{const saved=await post<{knowledge:{item:{id:string}}}>(`/research/threads/${pdf.workspace}/knowledge-items`,{kind:'reading-note',title:`PDF 批注 · ${pdf.path} · ${page}`,body,authorKind:'human',authorRef:`pdf:${pdf.workspace}:${pdf.digest}:${page}`},attempt.current.key);if(!saved?.knowledge?.item?.id)throw Error(tr('批注保存回执缺少标识，请用同一次请求核对保存结果。'));setComment('');setAnchor(null);setQuote('');setReload(n=>n+1);attempt.current=null;discussAnnotation(submittedAnchor,submitted,saved.knowledge.item.id)}catch(e){setError(e instanceof Error?e.message:String(e))}finally{setBusy(false)}}
 function discussAnnotation(target:PDFAnchor,text:string,annotationId:string){if(!pdf)return;requestReviewFeedback({id:crypto.randomUUID(),annotationId,designDiscussion:true,scope:{projectId:pdf.workspace,workspace:pdf.workspace},anchor:pdfFeedbackAnchor(pdf,target),body:text,at:new Date().toISOString()})}
 ...
 return <section aria-label={tr("PDF 阅读与批注")} data-xgc-role="pdf-reader" data-xgc-id={`${pdf.workspace}:${pdf.digest}`} className="flex min-h-0 flex-1 flex-col">
  ...
  <div className="min-h-0 flex-1 overflow-auto bg-inset p-3" data-xgc-role="pdf-viewport"><div ref={paper} data-xgc-role="pdf-page" data-xgc-id={`${pdf.digest}:page:${page}`} className="relative mx-auto bg-white" onMouseUp={selectText}>
   ...
   {(anchor||activeNote)&&<div data-xgc-role="pdf-annotation-editor" data-xgc-id="pdf-annotation-editor" className="absolute z-30 w-72 max-w-full rounded-lg border border-line bg-panel p-3 text-ink shadow-pop" style={{left:`${Math.min(lastRect?.x||0,Math.max(0,1-288/(paper.current?.clientWidth||380)))*100}%`,...(lastRect&&lastRect.y>.55?{bottom:`${(1-lastRect.y)*100}%`}:{top:`${Math.min(.75,(lastRect?.y||0)+(lastRect?.height||0))*100}%`})}} onMouseUp={e=>e.stopPropagation()}>
    ...
    {activeNote?<><p className="whitespace-pre-wrap text-secondary">{activeNote.comment}</p><Button className="mt-2" data-xgc-role="pdf-annotation-discuss" data-xgc-id={activeNote.id} onClick={()=>discussAnnotation(activeNote.anchor||{schema:'research.pdf-anchor/v1',kind:'page',page,rects:[],quote:'',context:''},activeNote.comment,activeNote.id)}>{tr('加入这次讨论')}</Button></>:<><textarea autoFocus data-xgc-role="pdf-annotation-comment" data-xgc-id="pdf-annotation-comment" aria-label={tr('PDF 批注')} placeholder={tr('写下对这一处的设计意见…')} className="ui-input" rows={3} disabled={busy} value={comment} onChange={e=>setComment(e.target.value)}/><div className="mt-2 flex gap-2"><Button variant="solid" data-xgc-role="pdf-annotation-submit" data-xgc-id="pdf-annotation-submit" loading={busy} disabled={!comment.trim()} onClick={()=>void annotate()}>{tr('提交批注并讨论')}</Button></div></>}
   </div>}
  </div></div>
 </section>
}
