...
/* frontmatter → 元信息块；[[wikilink]] → 可点击的内部链接 */
...
/* MarkdownView: 富文本渲染（主阅读面与右栏阅读共用，语义统一）：frontmatter 剥离、wikilink、代码高亮 */
export function MarkdownView({content}:{content:string}){
 const {knowledgeDocuments:notes,previewDocument,openPDF,flashPDF}=useWorkbench()
 const body=useMemo(()=>wikilink(splitFrontmatter(content).body),[content])
 /* 嵌进笔记的 PDF 批注锚点 → 回跳构建版本与页面位置 */
 ...
 async function openAnchor(){
  ...
  try{
   ...
   const match=versions.find(v=>v.digest===pdfAnchor.pdf!.digest)||versions[0]
   if(!match)throw Error(tr('这份稿件还没有可用的 PDF 构建。'))
   openPDF(match)
   flashPDF({buildId:match.buildId,page:pdfAnchor.page,rects:pdfAnchor.rects},match)
 ...
 </button>{anchorError&&<p role="alert" className="mt-1.5 text-caption text-ink-3">{anchorError}</p>}</div>}
 <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
  ...
  a:({href,children})=>href?.startsWith('#wiki/')?<button type="button" className="ui-wikilink" onClick={()=>{const target=decodeURIComponent(href.slice(6));const note=notes.find(n=>n.title===target||n.path.replace(/\.md$/i,'').endsWith(target));if(note)previewDocument({workspace:'academic',path:note.path,title:note.title})}}>{children}</button>:<a href={href} target="_blank" rel="noreferrer">{children}</a>,
 }}>{body}</ReactMarkdown>
 </>
}
export function Reader({onQuote}:{onQuote?:(text:string)=>void}){
 const {readingDocument:doc,closeDocument}=useWorkbench()
 const [content,setContent]=useState(''),[digest,setDigest]=useState(''),[error,setError]=useState(''),[loading,setLoading]=useState(false)
 useEffect(()=>{setContent('');setError('');if(!doc)return;const c=new AbortController();setLoading(true)
 ...
 return <div className="flex h-full min-h-0 flex-col">
  {onQuote&&<PageActions page="knowledge"><Button icon={Quote} disabled={!content} onClick={()=>onQuote(`文件：${doc.workspace}/${doc.path}\n版本：${digest}\n\n${content}`)}>{tr("引用到 Chat")}</Button></PageActions>}
  ...
  <article className="min-h-0 flex-1 overflow-auto"><div className="mx-auto w-full max-w-[720px] px-8 pb-20 pt-10">
   <header className="mb-8">
    <div className="flex items-center justify-between gap-3">
     ...
     <button type="button" onClick={closeDocument} className="flex h-6 shrink-0 items-center gap-1.5 rounded-md px-2 text-caption text-ink-3 transition-colors hover:bg-hover hover:text-ink-2" title={tr('返回图谱')}>
      <IconGraph size={12} strokeWidth={1.75}/>{tr('图谱')}
     </button>
    </div>
    <h1 className="mt-2 font-display text-[30px] leading-[1.2] tracking-tight">{doc.title}</h1>
   ...
   <div className="research-document break-words text-body leading-[1.75] text-ink-2">
    {loading&&!content?<p className="text-ink-3">{tr("正在读取…")}</p>:<MarkdownView content={content}/>}
   </div>
  </div></article>
 </div>
}
