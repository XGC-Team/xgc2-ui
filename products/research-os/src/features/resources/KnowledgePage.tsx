...
function KnowledgeView({onQuote,viewId='academic'}:KnowledgePageProps){
 ...
 if(readingDocument&&!blocked)return <div className="flex h-full min-h-0 flex-col">
  <div className="flex shrink-0 gap-2 border-b border-line p-2">
   ...
   {readingDocument.workspace==='academic'&&<button className="ui-btn" data-xgc-role="knowledge-local" onClick={()=>{setFilter('focus',readingDocument.path);closeDocument()}}>{tr('查看局部关系')}</button>}
 ...
 return <div className="relative h-full min-h-0" data-xgc-role="knowledge-workspace">
  ...
  <div className="absolute left-3 top-3 z-10 max-w-[calc(100%-1.5rem)] rounded-lg border border-line bg-panel/95 p-2">
   ...
   <div className="mt-2 flex flex-wrap items-center gap-2 text-caption">
    <label>{tr('未解析')} <select aria-label={tr('未解析目标过滤')} value={view.unresolved} onChange={e=>setFilter('unresolved',e.target.value as KnowledgeViewState['unresolved'])}><option value="include">{tr('包含')}</option><option value="only">{tr('仅未解析')}</option><option value="exclude">{tr('隐藏')}</option></select></label>
    <label>{tr('孤立节点')} <select aria-label={tr('孤立节点过滤')} value={view.orphans} onChange={e=>setFilter('orphans',e.target.value as KnowledgeViewState['orphans'])}><option value="include">{tr('包含')}</option><option value="only">{tr('仅孤立')}</option><option value="exclude">{tr('隐藏')}</option></select></label>
    <button data-xgc-role="knowledge-global" onClick={()=>{setView(previous=>({...emptyKnowledgeView(),camera:previous.camera}));setProjected(null)}}>{tr('清除筛选')}</button>
    <button data-xgc-role="knowledge-refresh" onClick={()=>{refresh();setRetry(n=>n+1)}}>{tr('刷新')}</button>
   </div>
   {view.focus&&<div className="mt-2 flex flex-wrap items-center gap-2 text-caption">
    <span>{tr('局部图')}</span><label>{tr('深度')} <input type="number" min="1" className="w-14" value={view.depth} onChange={e=>{const n=Number(e.target.value);if(Number.isSafeInteger(n)&&n>=1)setFilter('depth',n)}}/></label>
    <select aria-label={tr('局部关系方向')} value={view.direction} onChange={e=>setFilter('direction',e.target.value as KnowledgeViewState['direction'])}><option value="both">{tr('双向')}</option><option value="outbound">{tr('出链')}</option><option value="inbound">{tr('回链')}</option></select>
   ...
   <details className="mt-2 text-caption"><summary>{tr('通过列表检查节点')}</summary>
    <select size={6} className="mt-1 max-w-full" value={inspectId} aria-label={tr('检查知识节点')} onChange={e=>setInspectId(e.target.value)}>
