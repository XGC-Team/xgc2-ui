...
export async function openFeedbackAnchor(anchor: Anchor, scope: Scope): Promise<void> {
  ...
  if (anchor.kind === 'pdf') {
    if (anchor.origin === 'external') {
      s.openResource({ kind: 'original', workspace: anchor.workspace, path: anchor.path, digest: anchor.digest, page: anchor.page, quote: anchor.quote })
      return
    }
    ...
    s.openResource({kind: 'file', target: {projectId: scope.projectId, projectWorkspace: scope.workspace, workspace: anchor.workspace, path: anchor.path, view: 'files'}})
