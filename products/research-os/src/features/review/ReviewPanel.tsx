...
import { check, dependencyClosure, now, operationState, REVIEW_PATH, scopeKey, selectedGroups, uid, validPath, type Anchor, type FileRecord, type Operation, type Proposal, type Scope, type Target } from './review-model'
...
export function ReviewPanel({scope, tabId, onTitle}: {scope: Scope; tabId: string; onTitle: (title: string) => void}) {
  ...
  const addOperation = async () => {
    ...
    operations.filter(x => x.target.path === target.path).length && patchTarget(source.content, [...operations.filter(x => x.target.path === target.path), o], scope)
  ...
  return <section className="flex h-full min-h-0 flex-col" aria-label={t('反馈与修改审阅', 'Feedback and change review')} data-review-project={scope.projectId}>
    ...
    <div className="min-h-0 flex-1 space-y-4 overflow-auto p-3">
      ...
      {!proposal && seed && <fieldset disabled={busy || !api.book || api.auditUncertain} className="space-y-3">
        ...
        <label className="block text-secondary">{t('原始反馈', 'Original feedback')}<textarea required className="ui-input mt-1 w-full" value={body} onChange={e => setBody(e.target.value)}/></label>
        ...
        <div className="space-y-2 rounded-lg border border-line p-3">
          ...
          {source && <>
            ...
            {kind === 'text' ? <label className="block text-secondary">{t('在原始文本中选择要修改的片段', 'Select the exact range in raw text')}<textarea readOnly rows={6} className="ui-input mt-1 w-full font-mono" value={source.content} onSelect={e => setRange({start: e.currentTarget.selectionStart, end: e.currentTarget.selectionEnd})}/></label>
            ...
            <label className="block text-secondary">{t('修改后（该目标字段的完整值）', 'After (complete value of this field)')}<textarea rows={4} className="ui-input mt-1 w-full" value={after} onChange={e => setAfter(e.target.value)}/></label>
            <label className="block text-secondary">{t('理由与依据解释', 'Rationale and evidence explanation')}<textarea className="ui-input mt-1 w-full" value={reason} onChange={e => setReason(e.target.value)}/></label>
            ...
            <label className="block text-secondary">{t('额外待检查对象（每行一个，不自动修改）', 'Additional affected objects (one per line; no automatic edits)')}<textarea className="ui-input mt-1 w-full" value={impact} onChange={e => setImpact(e.target.value)}/></label>
        ...
        {promotion && <div className="space-y-2">
          ...
          <label className="block">{t('目标知识范围', 'Target knowledge scope')}<textarea className="ui-input w-full" value={knowledgeScope} onChange={e => setKnowledgeScope(e.target.value)}/></label>
          <label className="block">{t('适用条件与限制', 'Conditions and limitations')}<textarea className="ui-input w-full" value={conditions} onChange={e => setConditions(e.target.value)}/></label>
          <label className="block">{t('验证状态与待核验项', 'Verification state and remaining checks')}<textarea className="ui-input w-full" value={verification} onChange={e => setVerification(e.target.value)}/></label>
        </div>}
