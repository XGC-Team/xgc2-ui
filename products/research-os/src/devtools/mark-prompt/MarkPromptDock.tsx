...
import { useMarkPromptDockVisible } from './useMarkPromptDockVisible';
import { usePersistentState } from './usePersistentState';
import { writeClipboardText } from './clipboard';
import './family-scoped.css';
import './mark-prompt.css';
import {
...
export function MarkPromptDock({
  ...
  const [annotations, setAnnotations] = usePersistentState<DevAnnotation[]>('research.markPrompt.annotations.v1', [], isDevAnnotations);
  const [dragId, setDragId] = useState<string | null>(null);
  const [toolbarPos, setToolbarPos] = usePersistentState<ToolbarPositionPercent>('research.markPrompt.position', {
    xPercent: 1,
    yPercent: 1,
  }, isToolbarPosition);
      ...
      >
        <div className="mark-prompt-toolbar-row">
          <span
            ...
            title={tr("Drag mark prompt tools")}
          ...
          <ControlButton
            ...
            aria-label={tr("Copy generated prompt")}
            title={copyStatus === 'copied' ? tr("Copied") : copyStatus === 'failed' ? tr("Copy failed") : tr("Copy generated prompt")}
...
function AnnotationPin({
  ...
  return (
    ...
    >
      ...
      />
      <ControlButton size="compact" iconOnly disabled={disabled} onClick={onRemove} aria-label={tr("Remove mark")} dataXgcRole="mark-prompt-remove" dataXgcId="mark-prompt-remove">
        <X size={12} />
      </ControlButton>
    </div>
  );
