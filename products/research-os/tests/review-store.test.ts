...
describe('F3 review routing in the real Zustand store',()=>{
 beforeEach(()=>{useWorkbench.setState({resourceLayout:{version:1,tabs:[],active:{}},reviewIntents:[],reviewFocus:null,reviewDockOpen:false,projectId:'other-project',activeNav:'workflow'})})
 it('deduplicates review tabs per project/workspace without changing the selected project',()=>{
  const a=useWorkbench.getState().openResource({kind:'reviews',scope}),b=useWorkbench.getState().openResource({kind:'reviews',scope:{...scope}})
  expect(a).toBe(b);expect(useWorkbench.getState().resourceLayout.tabs).toHaveLength(1);expect(useWorkbench.getState().projectId).toBe('other-project')
 })
 it('keeps distinct project scopes separate',()=>{
  useWorkbench.getState().openResource({kind:'reviews',scope});useWorkbench.getState().openResource({kind:'reviews',scope:{...scope,projectId:'project-b'}})
  expect(useWorkbench.getState().resourceLayout.tabs).toHaveLength(2)
 })
 it('captures an immutable versioned feedback object without sending a chat prompt or opening a competing reviews tab',()=>{
  ...
  expect(useWorkbench.getState().resourceLayout.tabs).toHaveLength(0)
 })
 ...
 it('focus requests retain source identity and are not content writes',()=>{
  ...
  expect(useWorkbench.getState().reviewFocus?.anchor.digest).toBe('v1');expect(useWorkbench.getState().resourceLayout.tabs).toHaveLength(0)
 })
})
