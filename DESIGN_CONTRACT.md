...
- Cross-page geometry is a first-class contract: on list/table/form pages (Terminal, Operations, Audit, Settings, Robot assets, User scripts, Automations, Host pages, …) the content origin, toolbar position and height, list/table header position, and row height must be identical, so switching pages never moves a control under the operator's cursor. All such pages consume `--space-page-padding` for the content inset and the shared chrome/token steps for the rest. Exempt: multi-card workspace dashboards (Home, Experiments) whose layout is inherently card-based.
...
- The canonical list page layout is the audit-log pattern: one single-line toolbar (control-height, borderless, controls in one row) directly above the list surface, with the toolbar-to-list gap on the shared spacing rhythm. List pages (Terminal, Automations, Host Files, …) converge on this pattern instead of inventing multi-row or boxed toolbars.
