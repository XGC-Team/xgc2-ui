...
- A first-level panel header has the fixed height `--size-header-panel` (35px): `4px` top padding + a compact `--size-control-panel-header` (26px) action + `4px` bottom padding + the `1px` bottom hairline. `Panel` and `WorkspacePanel` use the same real `4px` padding on all four sides; the divider is not padding and must not be counted as optical compensation. Products must not size panel chrome or its controls locally. A framed `Panel` that renders a header uses the same `--space-xs` inset on `padding="default"` body as on the header; `--space-panel-padding` remains the inset for headerless or `chrome="flat"` default bodies, compact operator workspaces, and other non-header chrome. Products must not stack a second inset on that first body row. `padding="none"` is full-bleed only when the first body row has no extra inline padding; padding the thead, first row, pager, or chart legend with `--space-md` / `--space-lg` after `none` is still a second inset against a `--space-xs` header.
...
- Interactive `WorkflowStatusCard` tiles use clean control material with a short contact shadow, hover feedback, immediate pressed relief, keyboard focus, and a distinct disabled state. Relief does not move or resize the hit box. Informational article cards stay flat; product grids do not override the shared command tile material. All command tiles share a thin, permanently visible progress track in the same fixed overlay slot, including idle, zero, busy, disabled, running, and completed states. Disabled tiles dim their labels and lose elevation without fading their progress.
...
- AppShell desktop sidebar expand/collapse interpolates rail **width**. Inner brand/body/footer tracks stay at `--size-sidebar-expanded` and clip; do not snap label `opacity` or footer `display: none` at the start of the tween. Collapsed nav icons move to the center of the `--size-sidebar-collapsed` rail by interpolating `padding-inline` (`justify-content` cannot tween). Collapsed geometry remains a true `--size-sidebar-collapsed` box so the seam is honest. Mobile drawer overlay motion (transform/opacity covering content) is a separate compact-viewport pattern.
...
- Field hint labels (`.xgc-form-field-label`) use one typography everywhere, including standalone FormField rows outside `FormSection`: shared sans, base size, regular weight, muted color, and tight line height. The approved specimen is a Robot-asset numeric field such as MAV system ID. `FormSection` may change field layout only; it must not be the sole place that applies this type. Ordinary `Textarea` value text uses the same `--font-base` as `Input` and must not inherit an isolated field-container size. Code, JSON, and expression editors may set monospace explicitly. `ConfigSection` two-column settings rows may keep heading-colored labels as that family's geometry.
- Numeric configuration fields with a unit, a bounded range, or a discrete step use the shared number input with its complete, theme-colored increment/decrement control. Render a short unit as the input's muted suffix (`sec`, `px`, `m`, and so on), not as parenthesized text in the field label. Preserve native `spinbutton` semantics plus the authored `min`, `max`, and `step`; browser-painted spinners must not leak a foreign background or clipped arrows into the shared control, and product CSS must not hide or redraw the shared stepper. The stepper is a compact native-spinner-scale strip (`--size-icon-default` column): chevrons render from `--size-icon-2xs` boxes with `--stroke-thin`, painting ~7.5×3.75px hairline glyphs — subordinate to the 30px field, never a heavy arrowhead.
