...
- A first-level panel header has the fixed height `--size-header-panel`, based on the compact 34px XGC2 experiment panel. That 34px already includes the header's `--stroke-thin` bottom hairline, so padding-bottom is `calc(var(--space-xs) - var(--stroke-thin))` and compact `--size-control-panel-header` (26px) actions — including `PanelViewSwitcher` — sit equally between the top edge and that divider. Do not grow the header to 35px to "make room" for the line. Products must not size panel chrome or its controls locally.
